...
本文档记录了在 HarmonyOS（鸿蒙内核 1.12.0，aarch64）上编译 PyTorch v2.5.1 的完整适配过程。全部 12 个端到端测试均已成功通过。
...
2. **匹配 musl 签名**: HarmonyOS 使用 musl libc——需要根据 `/data/service/hnp/ohos-sdk.org/.../sysroot/usr/include/` 检查函数签名。

3. **对所有文件签名**: 每个 ELF 二进制文件都需要签名——在构建脚本中设置自动签名。

4. **交叉编译工具**: 当 cmake 检测到交叉编译时，它会在 `NATIVE_BUILD_DIR/bin/` 中查找原生工具。

5. **Python 存根模块**: PyTorch 的 Python 绑定是一个加载主库的薄存根——不要尝试将 libtorch_python.so 作为直接的 Python 扩展。
