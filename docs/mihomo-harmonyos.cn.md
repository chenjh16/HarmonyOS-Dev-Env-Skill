...
mihomo 是 Clash Meta 的核心实现，是一款功能强大的代理客户端。本文档记录了在 HarmonyOS 上编译、配置和运行 mihomo 的完整过程。

## 编译过程

### 1. 安装 Go 编译器

HarmonyOS 没有 gcc，需要下载官方 Go 发行版：
...
HarmonyOS 要求所有 ELF 二进制文件在执行前必须签名：
...
mihomo 仓库有两个分支：
- `main` - Python 客户端库（不是我们需要的）
- `Meta` - Clash Meta 核心（正确的分支）
...
mihomo 支持 GEOIP 和 GEOSITE 规则实现智能路由：
...
| 项目 | 订阅链接 | 说明 |
|---------|------------------|-------|
| Pawdroid/Free-servers | `https://raw.githubusercontent.com/Pawdroid/Free-servers/main/sub` | 每 6 小时更新 |
| crossxx-labs/free-proxy | `https://clash.crossxx.com/sub/vmess/xxx` | 每日配额限制 |
...
解决方案：使用不含 `GEOIP` 规则的配置，避免 GeoIP 数据库依赖。

### 2. Go 工具链权限拒绝

症状：`fork/exec ... permission denied`

解决方案：签名 Go 工具链中的所有 ELF 二进制文件，包括 `pkg/tool/linux_arm64/` 目录。
