...
This document records the complete adaptation process for compiling PyTorch v2.5.1 on HarmonyOS (HongMeng Kernel 1.12.0, aarch64). 15 of 15 end-to-end tests passed (torch.from_numpy() fixed via post-build patch; torch.det() fixed via OpenBLAS/LAPACK + supplement library).
...
make NOFORTRAN=1 TARGET=ARMV8 BINARY=64 \
  'CC=/data/service/hnp/bin/clang -B/storage/Users/currentUser/Claude/lib/linker_wrapper' \
...
clang -B/storage/Users/currentUser/Claude/lib/linker_wrapper \
...
**Important**: OpenBLAS build requires code signing at every step where executables are produced. Modify `Makefile.prebuild` to add `-B` linker wrapper and `binary-sign-tool` signing.
...
**Issue**: CMake 4.1.2 automatically runs `ldd` after linking executables. If `ldd` is not in PATH, all linking steps fail.
...
**Issue**: PyTorch compiles with `-fvisibility=hidden`, which hides `RefcountedMapAllocator::decref()`, `incref()`, and `at::internal::invoke_parallel()` from libtorch_cpu.so's dynamic symbol table. Even though the class has `TORCH_API` annotation, member function definitions without explicit visibility annotations inherit `-fvisibility=hidden`. libtorch_python.so references these symbols (undefined), causing "symbol not found" on load.

**Solution**: Create a supplement shared library (`libtorch_supplement.so`) providing stub implementations of the 3 missing symbols, and add it as a NEEDED dependency of `libtorch_python.so` using `patchelf --add-needed`:

```bash
# 1. Compile supplement library
clang++ -B$HOME/Claude/lib/linker_wrapper -shared \
  -o libtorch_supplement.so torch_supplement.o

# 2. Add as NEEDED to libtorch_python.so
patchelf --add-needed libtorch_supplement.so libtorch_python.so

# 3. Sign and install both libraries
...
### 15. NEEDED Library Path Format Difference (patchelf fix)

**Issue**: Ninja-built libraries use "lib/" prefix in NEEDED entries (e.g., `lib/libtorch_cpu.so`) while the original installation uses plain format (e.g., `libtorch_cpu.so`). This causes "Error loading shared library lib/libtorch_cpu.so" at runtime because the dynamic linker looks for the file literally at `lib/libtorch_cpu.so` relative to RUNPATH.

**Solution**: Use `patchelf --replace-needed` to fix all NEEDED entries after build:

```bash
for f in lib/*.so; do
  patchelf --set-rpath '$ORIGIN:/storage/Users/currentUser/.local/lib' $f
...
**Important**: Do NOT use `CMAKE_TOOLCHAIN_FILE` with `CMAKE_SYSTEM_NAME=Linux` — this triggers CMake's cross-compilation mode, which causes `try_run()` failures because CMake cannot execute test binaries. Instead, use a lightweight toolchain file that only sets compilers and linker wrapper, without `CMAKE_SYSTEM_NAME`.
...
15 of 15 tests passed — all tests functional:
...
13. torch.det() ✓ (LAPACK/OpenBLAS enabled)
14. torch.from_numpy() ✓ (fixed via post-build patch with USE_NUMPY)
...
10. **Sleef header generation bug**: When using NATIVE_BUILD_DIR with IMPORTED sleef tools, CMake's `sleef_concat_files` generates a ninja rule that misses SIMD section files. Manually concatenate all section files to create `sleef.h`.

11. **cpuinfo static library incomplete**: `libcpuinfo.a` may contain only 4 objects instead of 17+. Rebuild with `ar rcs` using all compiled .o files to include ARM Linux init functions.

12. **visibility=hidden symbol export**: PyTorch compiles with `-fvisibility=hidden`. Member functions in TORCH_API classes without explicit visibility annotations get hidden. Create `libtorch_supplement.so` with stub implementations and add as NEEDED dependency via `patchelf --add-needed`.

13. **NEEDED path prefix fix**: Ninja-built libraries use "lib/" prefix in NEEDED entries (e.g., `lib/libtorch_cpu.so`). Use `patchelf --replace-needed` to strip the prefix. Also set RUNPATH to `$ORIGIN:/storage/Users/currentUser/.local/lib`.
