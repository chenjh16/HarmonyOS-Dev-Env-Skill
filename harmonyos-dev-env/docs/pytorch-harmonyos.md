...
13. **NEEDED path prefix fix**: Ninja-built libraries use "lib/" prefix in NEEDED entries (e.g., `lib/libtorch_cpu.so`). Use `patchelf --replace-needed` to strip the prefix. Also set RUNPATH to `$ORIGIN:$HOME/.local/lib`.
